 ...
 * @category Encryption
 */
export async function aesGcmEncrypt<T extends string|Uint8Array>(content: T, password: string|Uint8Array, prefix?: string | Uint8Array, ivLen = 5): Promise<T> {
    const pwUtf8 = typeof password === 'string' ? new TextEncoder().encode(password) : password // encode password as UTF-8
    const pwHash = await crypto.subtle.digest('SHA-256', pwUtf8) // hash the password

    // todo: default ivLen=5 is 40 bits. NIST recommends 96-bit (12 bytes) for AES-GCM. Change default to 12 and migrate existing tokens.
 ...
 * @category Encryption
 */
export async function aesGcmDecrypt<T extends string|Uint8Array>(ciphertext: T, password: string|Uint8Array, ivLen = 5): Promise<T> {
    const pwUtf8 = typeof password === 'string' ? new TextEncoder().encode(password) : password // encode password as UTF-8
    const pwHash = await crypto.subtle.digest('SHA-256', pwUtf8) // hash the password
